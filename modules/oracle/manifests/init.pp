# Pre-requisites for the Oracle installation
class oracle::server {
  exec { 'apt-update':
  ...
  # If we're running on Windows, then Git may have converted line endings to
  # CRLF upon cloning the # repository. Here we use dos2unix to make sure
  # that they are LF.
  exec {
    'dos2unix chkconfig':
...
# Create swapfile for the Oracle installation
class oracle::swap {
  exec {
    'create swapfile':
